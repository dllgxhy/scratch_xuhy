...
//		var url:String = getCdnStaticSiteURL() + 'medialibraries/' + libraryType + 'Library.json';
	...
	//该段代码从王辉处拷贝，采用该代码后，可以正常显示背景图片小图标 
	public function getThumbnail2(md5:String, w:int, h:int, whenDone:Function):URLLoader {
	...
	/*此处经过更改，在调用背景库的时候，无法显示小图标，原因是 downloadThumbnail，与 getThumbnail2的区别，
	*/
...
//		return downloadThumbnail(url, w, h, whenDone); //xuhy 更改
		return getThumbnail2(idAndExt,w,h,whenDone);   //xuhy 添加
	}
